...
import FileUploadCompact from "@/components/compact-upload-edit";
import {
  AlertDialogContent,
  AlertDialogTrigger,
  AlertDialogAction,
...
export default function EditProductPage() {
  ...
  const getProductDetail = async () => {
    try {
      const response = await productApi.getProductDetail_EditProduct(productId);
      preloadProductData(response.data);
      setIsLoading(true);
    } catch (error) {
  ...
  const preloadProductData = async (productData) => {
    if (!productData) {
      return;
    }
    const productProperties = JSON.parse(productData.propertyList);
    const productDescription = JSON.parse(productData.description);
    const vTable = productData.variantSellingPoint.map((dataRow) => ({
      stock: dataRow.stock.toString(),
      sellingPrice: dataRow.price.toString(),
      sellerSku: dataRow.seller_sku,
      v1_name: dataRow.optionValue1,
      v1_tempId: dataRow.optionId1,
      v2_name: dataRow.optionValue2,
      v2_tempId: dataRow.optionId2,
      isInUse: dataRow.isInUse,
      isOpenToSale: dataRow.isOpenToSale,
    }));
    console.log("VT: ", vTable);
    setProductName(productData.name);
    setSelectedCategory(productData.categoryId);
    setThumbnailFile(productData.thumbnailURL);
    setProductDescription(productDescription);
    setProductProperties(productProperties);
    setVariant1(productData.variant1);
    setVariant2(productData.variant2);
    setVariantSellingPoint(vTable);
    setIsDrafted(productData.isDrafted);
  };
  useEffect(() => {
    console.log("VSP: ", variantSellingPoint);
  }, [variantSellingPoint]);
  useEffect(() => {
    if (!productId) {
      return;
  ...
  return (
    <div className="page-layout">
      <h1>{!isDrafted ? "Chỉnh sửa sản phẩm" : "Chỉnh sửa bản nháp"}</h1>
      {/* Big layout!*/}
      {/*Content*/}
      <div className="grid grid-cols-[75%_25%] gap-4">
        <div className="flex flex-col gap-6">
          ...
          >
            ...
            {isDrafted && (
              <Button onClick={handlePublishSubmit} variant={"outline"}>
                Lưu lại bản nháp
              </Button>
            )}
            <Button className={"bg-blue-500"} onClick={handlePublishSubmit}>
              {!isDrafted ? "Gửi bản cập nhật" : "Gửi đi"}
            </Button>
          </div>
        </div>
...
function VariantAndSellingBlock({
  ...
  let total = 0;
  const v1Exist = variant1 && variant1.valueList.length > 0;
  const v2Exist = variant2 && variant2.valueList.length > 0;
  if (v1Exist) total++;
  if (v2Exist) total++;
